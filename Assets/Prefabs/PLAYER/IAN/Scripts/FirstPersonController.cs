...
[RequireComponent(typeof(PlayerLook), typeof(PlayerMovement), typeof(PlayerSlide))]
[RequireComponent(typeof(PlayerCrouch), typeof(PlayerHeadBob))]
public class FirstPersonController : MonoBehaviour
{
    private PlayerLook lookModule;
    private PlayerMovement movementModule;
    private PlayerSlide slideModule;
    private PlayerCrouch crouchModule;
    private PlayerHeadBob headBobModule;

    void Awake()
    {
        lookModule = GetComponent<PlayerLook>();
        movementModule = GetComponent<PlayerMovement>();
        slideModule = GetComponent<PlayerSlide>();
        crouchModule = GetComponent<PlayerCrouch>();
        headBobModule = GetComponent<PlayerHeadBob>();
    ...
    void Update()
    {
        ...
        slideModule.HandleSlide();
        crouchModule.HandleCrouch();

        // Sincronizar estados entre módulos si es necesario
        lookModule.IsSliding = slideModule.IsSliding;
        lookModule.HandleLook();

        headBobModule.HandleHeadBob();
    }

    // Control de entrada externo
    public void SetControl(bool enabled)
    {
        lookModule.IsLookEnabled = enabled;
        movementModule.IsMoveEnabled = enabled;
    }
...
//    [Tooltip("Movimientos básicos del Player")]
...
//    [Tooltip("Intensidad del tilt lateral al moverse horizontalmente")]
//    public float lateralTiltAmount = 2f;

//    [Header("Speed Settings")]
//    [Tooltip("Velocidades básicas del Player")]
...
//                    Quaternion.Euler(yVelocity - slideTiltAngle, 0f, 0f),
//                    Time.deltaTime * 10f
//                );
//            }
//            else
...
//            // Aquí usamos la variable pública lateralTiltAmount en lugar del valor fijo
//            recoil.z = moveInput.x * -lateralTiltAmount;
//        }
//        else
//        {
//            bobTimer = 0f;
